...
class sjf{
	...
	public static void main(String[] args) throws Exception{
		...
		for(int time=0;time<10;time++){
			...
			if(current_running_pid == -1){
				...
				Process processToSchedule = getShortestProcess(); 			 //get the process with shortest burst time
	...
	public static Process getShortestProcess(){
		int min_burst_time = 99999;
		Process shortestProcess = null;
		Iterator i = arrivedProcess.iterator();
		while(i.hasNext()){
			Process p =(Process)i.next();
			if(min_burst_time > p.getBurstTime()){
				min_burst_time = p.getBurstTime();
				shortestProcess = p;
			}
		}
